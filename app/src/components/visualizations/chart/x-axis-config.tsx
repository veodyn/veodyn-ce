...
// Type sizes come from axis-config so this axis matches the other four
// renderers and the chrome guard has one place to check. AXIS_TICK.fontSize is
// 12, ANNOTATION_LABEL_FONT_SIZE is 11.
...
/**
 * Axis props for the x column, and the rows to draw against them. A date column
 * gets a real time scale, so ticks land on round boundaries and a gap in the data
 * reads as a gap; a category axis gives every row equal width instead.
 *
 * `categorical` opts out for bar charts, whose bars need discrete bands to size
...
export function planXAxis(
  ...
  return {
    ...
    props: {
      ...
      // TimeAxisTick renders its own text, but recharts measures a tick's width
      // by running tickFormatter over the raw value (getTicks.js) against
      // minTickGap, and without this it measures a 13-digit epoch. Same formatter
      // as the tick element, so a 12-hour format (three characters wider than
      // 24-hour) is measured as what the axis actually draws.
      tickFormatter: (value: unknown) => formatCategoryTick(value, granularity, patterns),
    },
...
interface TimeAxisTickProps {
  granularity: TimeGranularity
  // Threaded down from the renderer rather than read from a hook: recharts
  // clones this element per tick, and the plan's tickFormatter above needs the
  // same patterns from outside React, or the drawn and measured labels differ.
  patterns: DisplayPatterns
...
/**
 * A tick label plus, on a second line, the coarser unit it leaves ambiguous. The
 * context line is drawn only where it changes (the first tick, then each midnight
 * or new year), so an intraday axis states the date once.
 *
 * The first tick suppresses its date when the NEXT tick starts a period: the two
 * land one tick gap apart, and a ten-character date is wider than that, so they
 * overprinted into "2026-07-2026-07-25".
 */
