// The one link the Captures board makes into enterprise territory.
//
// Captures are community and alerts are not, so the Recipients button on an
// armed capture is the single place the community board addresses a route it
// may not have. What is asserted here is that it asks first.
...
/** An armed capture: a declared expectation, and an alert derived from it. */
const ARMED: Capture = {
  id: 'orders',
  name: 'Orders capture',
  source: 'clickhouse',
...
describe('ExpectedIntervalControl', () => {
  ...
  it('links an armed capture to the alert that pages for it', () => {
    renderWithProviders(<ExpectedIntervalControl capture={ARMED} />)
