...
//
// Asserted on source text rather than on a bundle, so a forbidden import fails
// when it is written rather than when it ships.
//
// Packages and files are both discovered from the directory rather than listed,
// so a second package is covered the moment it arrives.
...
// Every source extension the build can consume, not just .ts/.tsx: tsconfig.json
// enables allowJs, so an overlaid package can ship a renderer.jsx that index.ts
// imports, and it has to be scanned exactly like a .ts file.
 ...
 * failure names the package as well as the file. Recursive: every package is
 * flat today, but nothing stops one from having subdirectories.
 */
...
// the header comment of camera-slider.ts contains the literal "@/components/ui/*",
// and a block-comment regex reads that stray "/*" as a comment start and eats
// every import in the file.
//
// Known gaps: a template-literal specifier (import(`./foo/${x}`)) is not seen at
// all, since importsIn() only matches a quoted string, and stripComments() does
// not understand string literals, so a string containing "/*" makes it swallow
...
// identifier reads as the keyword: `'effective_from']` matched, and the capture
// ran to the next quote in the file, so a column name reported the whole plugin
// declaration as a forbidden import.
...
// A guard that matches nothing is a guard that cannot fail. Everything below pins
// that this one sees real code, covers every package, and would report a
// forbidden import rather than quietly skip it.
describe('plugin import boundary, self-checks', () => {
  // Not an exact list: a new package must be covered the moment it lands, with
  // nobody remembering to edit this file.
  ...
  // contributes no files leaves its own boundary unchecked while the suite stays
  // green.
  ...
  it('finds every plugin source file', () => {
    ...
    // index.ts). Not 0: a package that stops contributing files must still fail.
  ...
  // The guard has to see real imports and only real imports: an over-eager
