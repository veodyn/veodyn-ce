/**
 * Same-origin proxy for the capture-metadata backend (Captures).
 *
 * GET /api/captures -> CATALOG_API_URL/captures (api/veodyn_api/routers/captures.py).
 *
 * 503s when CATALOG_API_URL is unset (see ../catalog/route.ts), which is the
 * one status useCaptures falls back to fixtures on.
 */
...
export async function GET(request: Request) {
  return forward(request, '/captures')
}

/**
 * Declaring how often a capture is expected to deliver. The capture id rides in
 * a query parameter: this route has no dynamic segment, and the id is a
 * warehouse table name (`historical.q_regional_demo_bikeshare_stations_32`).
 */
export async function PUT(request: Request) {
  ...
  // Two writes on one capture, named rather than inferred from the body shape,
  ...
  return forward(request, `/captures/${encodeURIComponent(captureId)}/${resource}`, {
    method: 'PUT',
    body,
