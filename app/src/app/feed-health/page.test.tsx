...
describe('FeedHealthPage', () => {
  it('renders a row per feed with a status label (icon + text, not color alone)', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Feed Health' })).toBeInTheDocument()
  ...
  // Every feed on the stage instance reported a cadence of "not scheduled",
  // sitting next to a Last received of "59 seconds ago". The column carried no
  // information, and worse, deriveFeedStatus silently stops aging a feed it
  // cannot parse a cadence for, so the verdict beside it was the upstream's
  // claim repeated back rather than anything checked.
  it('says when there is no cadence to judge a feed against', async () => {
    useMockDataStore.setState({
      feeds: [
        {
          id: 'unscheduled',
  ...
  it('keeps the cadence and claims nothing extra when the feed does declare one', async () => {
    useMockDataStore.setState({
      feeds: [
        {
          id: 'scheduled',
  ...
  // row, which would say the feeds affect nothing.
  ...
  // is checked, because deriveFeedStatus has no period to age against and falls
  // back to repeating whatever the upstream last claimed.
  it('lets an operator declare how often a feed should deliver, and then checks it', async () => {
