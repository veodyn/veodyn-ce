 ...
 *
 * Comments ARE stripped here, unlike feature-boundary-scan.ts: the thing being
 * looked for is a URL, and a URL named in a sentence is how a comment explains
 * what a component is for (heatmap-grid-chrome.ts names `/present` in prose).
 * Flagging one would fail closed on correct code.
 *
 * Two known limits, both accepted. A regular expression containing a literal
 * `//` reads as the start of a line comment, so the rest of that line is
 * skipped; a backtick inside a template literal's `${}` expression ends the
 * literal early. Both fail by reporting LESS rather than something untrue.
 */
 ...
 * about its own allowlist: `/reports` must not claim `/reportsomething`.
 */
...
export function isSkipped(rel: string): boolean {
  ...
  // Nothing is present in this tree for this branch to skip; it is what an
  // overlay build needs, where the guard must stay about COMMUNITY modules
  // naming an enterprise route rather than about the feature naming its own.
  ...
  // owned by the pack along with the page.
