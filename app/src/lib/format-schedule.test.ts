...
const UNSET = { text: 'No refresh schedule', state: 'unset' }
...
describe('formatQuerySchedule', () => {
  ...
  it('treats a zero or absent interval as unscheduled, which is how Redash spells never', () => {
    expect(formatQuerySchedule(schedule({ interval: 0 }), NOW)).toEqual(UNSET)
    expect(formatQuerySchedule(schedule({ interval: null }), NOW)).toEqual(UNSET)
    // A leftover time or until does not make a schedule out of no interval.
    expect(formatQuerySchedule(schedule({ interval: 0, time: '06:00' }), NOW)).toEqual(UNSET)
  })
  ...
  it('adds the day a weekly schedule runs on', () => {
    ...
    ).toEqual({ text: 'Refreshes weekly on Monday at 06:00', state: 'set' })
  ...
  it('reports a schedule whose until has passed as ended, not as live', () => {
    ...
    ).toEqual({ text: 'Schedule ended 07/01/26', state: 'ended' })
  })
  ...
  it('keeps describing a schedule whose until is still ahead', () => {
    ...
    expect(ahead).toEqual({ text: 'Refreshes every 15 minutes', state: 'set' })
  })
  ...
  it('measures until against the clock it was handed, not a fixed date', () => {
    ...
    expect(formatQuerySchedule(ending, Date.parse('2026-07-26T12:00:00Z')).state).toBe('set')
    expect(formatQuerySchedule(ending, Date.parse('2026-07-28T12:00:00Z')).state).toBe('ended')
  })
