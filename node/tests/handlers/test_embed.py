"""CSP behavior around embeddable content.

TestEmbedVisualization and TestPublicDashboard used to live here too,
covering handlers/embed.py's "/embed/query/<id>/visualization/<id>" and
"/public/dashboards/<token>" routes. Both routes served the fork's own SPA
shell and were removed once the product UI (a separate Next.js app) took
over serving those pages; there is no HTML page left in this app to embed at
either path. TestAPIPublicDashboard below, which covers the JSON API route
those pages actually fetch from, is unaffected and stays.
"""
