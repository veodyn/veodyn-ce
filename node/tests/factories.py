...
# `webhook`, and the type matters rather than being decoration.
# NotificationDestination.to_dict() reaches through get_destination() to call
# icon() on the registered class, so a factory naming a type this edition does
# not enable makes every /api/destinations test die on None. It has to be one of
# default_destinations in redash/settings/__init__.py.
destination_factory = ModelFactory(
    ...
    type="webhook",
    options=lambda: ConfigurationContainer.from_json('{"url": "https://example.com/webhook"}'),
)
...
class Factory:
    ...
    def grant_permission(self, permission, group=None):
        """Add a permission string to a group, the way cli/groups.py does.

        Rebinds the list rather than appending, because Group.permissions is a
        plain ARRAY with no mutation tracking, so an in-place append never
        reaches the database.
        """
        group = group or self.default_group
        group.permissions = group.permissions + [permission]
