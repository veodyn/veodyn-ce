...
class TestHMACAuthentication(BaseTestCase):
    ...
    def test_user_api_key(self):
        user = self.factory.create_user(api_key="user_key")
        # Org scoped, like test_no_query_id above: with the SPA catch-all
        # route gone, an unscoped path no longer resolves to anything (it
        # used to match that route's wildcard regardless of what came after
        # the first segment, which is not what this test means to exercise).
        path = "/{}/api/queries".format(self.query.org.slug)
        models.db.session.flush()
...
class TestJWTAuthentication(BaseTestCase):
    ...
    def test_jwt_no_token(self):
        # A real, login-gated API resource, not the SPA catch-all this test
        # used to lean on (removed along with the SPA it served). Unlike the
        # catch-all's page path, an "/api/" path answers 404 rather than a
        # 302 to the login page when unauthorized: see redirect_to_login in
        # redash/authentication/__init__.py, which this task did not change.
        response = self.get_request("/api/data_sources", org=self.factory.org)
    ...
    def test_jwt_from_pem_file(self):
        ...
        response = self.get_request("/api/data_sources", org=self.factory.org, headers={self.token_name: token_data})
        self.assertEqual(response.status_code, 200)
