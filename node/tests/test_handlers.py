...
class TestAuthentication(BaseTestCase):
    # /api/session, not the bare "/" this used to hit: "/" no longer gates on
    # the session (handlers/static.py), and an unauthorized "/api/" path
    # answers 404, not a redirect (redirect_to_login, unrelated to this task).
    def test_responds_with_success_for_signed_in_user(self):
        with self.client as c:
            with c.session_transaction() as sess:
                sess["_user_id"] = self.factory.user.get_id()
            rv = self.client.get("/default/api/session")

            self.assertEqual(200, rv.status_code)

    def test_404s_for_nonsigned_in_user(self):
        rv = self.client.get("/default/api/session")
        self.assertEqual(404, rv.status_code)

    def test_404s_for_invalid_session_identifier(self):
        with self.client as c:
            with c.session_transaction() as sess:
                sess["_user_id"] = 100
            rv = self.client.get("/default/api/session")
...
# IndexTest used to live here: the bare index route, /dashboard/<slug>, and
# arbitrary paths only reachable through the catch-all /<path:path> handing
# off to the SPA's own client-side router. All were removed once the product
# UI took over serving those pages. tests/test_headless_boot.py covers what
# "/" does now.
