...
def hmac_load_user_from_request(request):
    ...
    if signature and time.time() < expires <= time.time() + 3600:
        ...
        if query_id:
            ...
            # The archived check has to be here as well as in
            # has_access_to_object, because this ApiUser carries the query's own
            # data source groups (below) rather than an empty set. Routes that
...
def log_user_logged_in(app, user):
    # The one server-side event that does not go through
    # handlers.base.record_event, so it carries the provenance stamp itself. A
    # login left unstamped would read as unknown provenance forever, and logins
    # are the rows an auditor looks at first.
    event = {
        "org_id": user.org_id,
        "user_id": user.id,
        "action": "login",
        ...
        models.CLIENT_SUBMITTED_KEY: False,
    }
