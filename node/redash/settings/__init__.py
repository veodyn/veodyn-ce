...
# Defaults to the fork's own templates/ directory, not STATIC_ASSETS_PATH: the
# only templates this app renders (login, setup, emails, multi_org.html) live
# there, so template lookup no longer depends on a client build existing.
FLASK_TEMPLATE_PATH = fix_assets_path(os.environ.get("REDASH_FLASK_TEMPLATE_PATH", "templates"))
...
# Where the product UI lives. The fork used to be its own UI and HOST was
# both the API origin and the place a /queries/<id> link resolved. With the
# client removed those are different hosts, and every generated link (alert
# notifications, failure reports, invites, password resets) belongs to the UI.
#
# Left unset by default rather than falling back to HOST here: this value also
# drives handlers/static.py's redirect at "/", and HOST is this app's OWN
# origin, so defaulting to it would send "/" straight back to "/" forever.
# Link generation still gets a safe fallback: base_url() (redash/utils/__init__.py)
# falls back to settings.HOST itself at call time, so an unset UI_BASE_URL
# behaves exactly as before for alerts, invites, and password resets, without
# ever making this variable truthy when nobody configured it.
UI_BASE_URL = os.environ.get("REDASH_UI_BASE_URL", "")
...
default_query_runners = [
    ...
    "redash.query_runner.openweathermap",
    "redash.query_runner.go511",
    "redash.query_runner.trafficland",
    "redash.query_runner.waze",
...
# Destinations
#
# The two this edition ships, and the reasoning is in docs/connector-curation.md:
# `email` needs no third-party account, and `webhook` is the generic escape
# hatch every other channel can be rebuilt on. Anything beyond them is installed
# into the image and named in REDASH_ADDITIONAL_DESTINATIONS, which is read a
# few lines down; the loader treats such a module exactly like one of these.
...
# ClickHouse (historical capture)
# An empty HISTORICAL_CLICKHOUSE_URL means capture is globally disabled.
HISTORICAL_CLICKHOUSE_URL = os.environ.get("REDASH_HISTORICAL_CLICKHOUSE_URL", "")
HISTORICAL_CLICKHOUSE_USER = os.environ.get("REDASH_HISTORICAL_CLICKHOUSE_USER", "default")
HISTORICAL_CLICKHOUSE_PASSWORD = os.environ.get("REDASH_HISTORICAL_CLICKHOUSE_PASSWORD", "")
HISTORICAL_CLICKHOUSE_DATABASE = os.environ.get("REDASH_HISTORICAL_CLICKHOUSE_DATABASE", "historical")
# Id of the Redash data source pointing at the historical store, if registered; used to
# guard against capturing queries run against the historical warehouse itself.
HISTORICAL_DATA_SOURCE_ID = int_or_none(os.environ.get("REDASH_HISTORICAL_DATA_SOURCE_ID"))

# Names of extra groups that every newly created data source is attached to, on top of
# the org's default group that DataSource.create_with_group always attaches.
#
# Upstream ties data source access to the default group alone, so the only way to give a
# machine account access to data sources is to leave it in the default group, which also
# carries create/edit query, create/edit dashboard and list_users. A service account that
# only has to run queries then holds all of that, and there is no way to drop it without
# also dropping its access to every data source.
#
# Naming a group here decouples the two: the account can sit in a group carrying only the
# permissions it needs and still see data sources created later, without an admin
# remembering a manual step in the groups UI each time. Left empty, which is the default,
# nothing changes and creation behaves exactly as upstream.
ADDITIONAL_DATA_SOURCE_GROUPS = array_from_string(os.environ.get("REDASH_ADDITIONAL_DATA_SOURCE_GROUPS", ""))
