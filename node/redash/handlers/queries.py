...
class BaseQueryListResource(BaseResource):
    def get_queries(self, search_term):
        # include_drafts=False so a draft is listed only for its own author.
        # Upstream passes True here, which returned everyone's drafts and left
        # is_draft with no effect on this endpoint at all: publishing a query
        # changed nothing anybody could see. The model still includes the
        # caller's own drafts, via or_(is_draft.is_(False), user_id == user_id),
        # so authors keep finding their unfinished work here.
        #
        # This is a listing rule, not an access rule. QueryResource.get gates on
        # data source access and never looks at is_draft, so a draft remains
        # openable by link to anyone who could open it before.
        if search_term:
            ...
            results = models.Query.all_queries(
                self.current_user.group_ids, self.current_user.id, include_drafts=False
            )
...
class QueryResource(BaseResource):
    ...
    def get(self, query_id):
        ...
        # attached here instead of in serialize_visualization: only a caller
        # who could mint or revoke it (an admin, or this query's owner) may
        # read it back, and every other user who can view the query gets the
        # same response without it. DashboardResource.get attaches api_key in
        # the handler for the same reason. Reading it back is what lets the
        # embed dialog offer the link it already has instead of minting a
        # second one.
        if is_admin_or_owner(q.user_id):
            tokens = {vis.id: models.ApiKey.get_by_object(vis) for vis in q.visualizations}
            for serialized in result["visualizations"]:
                api_key = tokens.get(serialized["id"])
                if api_key:
                    serialized["api_key"] = api_key.api_key
