...
def public_dashboard_url(token, org_slug=None):
    """The address a dashboard's public token resolves to.

    Built off the index route rather than a dedicated endpoint: the route
    that used to answer "/public/dashboards/<token>" (handlers/embed.py) was
    part of the fork's own SPA shell and was removed once the product UI took
    over serving that page. The shape (this org's index address plus
    "public/dashboards/<token>") is kept unchanged so a public_url value
    still parses the same way for anything that reads the token off its
    tail, even though this app no longer answers the path itself.
    """
    if settings.MULTI_ORG:
        index_url = url_for("redash.index", org_slug=org_slug, _external=True)
    else:
        index_url = url_for("redash.index", _external=True)

    return "{}public/dashboards/{}".format(index_url, token)


class BaseDashboardListResource(BaseResource):
    def get_dashboards(self, search_term):
        if search_term:
            return models.Dashboard.search(
                self.current_org,
            ...
            return models.Dashboard.all(self.current_org, self.current_user.group_ids, self.current_user.id)
    ...
    def get(self):
        ...
        results = self.get_dashboards(search_term)
...
class DashboardListResource(BaseDashboardListResource):
...
class DashboardArchiveResource(BaseDashboardListResource):
    # Without this endpoint DELETE /api/dashboards/<id> is unrecoverable: it
    # only sets is_archived, and every listing hides archived dashboards, so the
    # dashboard is still there but nothing can name it again. The permissions,
    # the draft rule and the ordering are the ones inherited from the base
    # class, because this is the same listing over a different set of rows.
    def get_dashboards(self, search_term):
        if search_term:
            return models.Dashboard.search(
                self.current_org,
...
class DashboardResource(BaseResource):
    ...
    def get(self, dashboard_id=None):
        ...
        if api_key:
            response["public_url"] = public_dashboard_url(api_key.api_key, org_slug=self.current_org.slug)
    ...
    def delete(self, dashboard_id):
        ...
        # The public link is revoked in this same transaction by
        # revoke_share_tokens_when_target_is_archived, on the flush this commit
        # runs. Not written out here: post() can archive through its generic
        # field update as well, so a revoke that lived in this method would
        # cover one of the two ways a dashboard gets deleted.
        models.db.session.add(dashboard)
...
class PublicDashboardResource(BaseResource):
    # Deliberately not login_required: a token that does not resolve leaves the
    # reader anonymous, and that request still has to reach this handler so the
    # refusal gets recorded. Every outcome answers 404 either way.
    ...
    def get(self, token):
        ...
        """
        api_key, dashboard, outcome = models.ApiKey.resolve_share_token(token, models.Dashboard, self.current_org)

        if outcome == "ok" and self.current_org.get_setting("disable_public_urls"):
            outcome = "disabled"

        # Recorded against the org that owns the key, not the org named in the
        # route. A token redeemed under someone else's slug is an event for the
        # tenant whose link it is, and that tenant is the one whose settings
        # decided whether the link may be read at all.
        record_public_read(
...
class DashboardShareResource(BaseResource):
    @require_any_of_permission(("admin", "publish_dashboard"))
    def post(self, dashboard_id):
        """
        ...
        # Checked on minting and not only on reading, otherwise a link can be
        # created while the switch is off and go live the moment it flips back.
        ...
        # Idempotent on purpose: sharing an already shared dashboard returns the
        # token it already has instead of minting a second one that revocation
        # would never reach. Assigning expires_at here is therefore how the
        # terms of that one link change, and an absent expires_at clears any
        # expiry a previous share set rather than inheriting it.
        try:
            api_key = models.ApiKey.get_or_create_for_object(dashboard, self.current_user)
        except models.ApiKey.TargetArchived:
            # Archiving is the product's delete, so there is nothing here to
            # publish. 404 and not 400, because the answer is that the
            # dashboard is gone rather than that the request was wrong. Raised
            # from under the per object lock, so it also covers an archive that
            # commits while this request is queued behind it.
            abort(404)
        ...
        public_url = public_dashboard_url(api_key.api_key, org_slug=self.current_org.slug)
    ...
    def delete(self, dashboard_id):
        ...
        dashboard = models.Dashboard.get_by_id_and_org(dashboard_id, self.current_org)
        require_admin_or_owner(dashboard.user_id)
        # Every live key, not just the first: a dashboard shared twice before
        # minting became idempotent carries an orphan the UI cannot see, and
        # revoking one at a time would leave it working.
        revoked = models.ApiKey.deactivate_for_object(dashboard)
