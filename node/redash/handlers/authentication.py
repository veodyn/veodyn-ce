...
# ---------------------------------------------------------------------------
# JSON API endpoints for external admin frontends (e.g. luka_admin)
...
def api_invite_get(token, org_slug=None):
    ...
    return json_response({"user": {"name": user.name, "email": user.email}})
...
def api_invite_post(token, org_slug=None):
    ...
    if len(password) < 6:
        return json_response({"error": "short_password", "message": "Password must be at least 6 characters."}), 400
...
def api_reset_get(token, org_slug=None):
    try:
        ...
        return json_response({"error": "invalid", "message": "Reset link is invalid."}), 400
...
def config(org_slug=None):
    return json_response({"org_slug": current_org.slug, "org_name": current_org.name, "client_config": client_config()})
