...
**node**, the query backend: a headless query, dashboard and scheduling API. Flask backend with SQLAlchemy ORM. Background jobs via Redis Queue (RQ). Data sources come from a pluggable query runner system. The default list has 44 entries, 17 of them the transportation connectors. The React client and `viz-lib` visualization library that used to live in this tree were deleted; the product UI is the separate Next.js app in `app/`.
