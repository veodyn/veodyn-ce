...
| Static GTFS | `gtfs_static` | Every `.txt` table in a GTFS zip archive, which covers the schedule (stops, routes, trips, stop_times, calendar), GTFS-Pathways (`pathways`, `levels`) and the GTFS-Flex tables (`booking_rules`, `location_groups`, `location_group_stops`, and the pickup and drop-off window columns of `stop_times`); `{"resource": "list"}` names the tables it holds, `{"table": "stops"}` returns one, with optional `columns` and `filter`. A Flex archive's `locations.geojson` is served as the `locations` table, one row per zone with its geometry, or as one GeoJSON cell with `"format": "featurecollection"` | Archive URL (`http(s)://`, required); max rows per query (default 100000) |
...
| GOFS On-Demand | `gofs` | An on-demand (microtransit, paratransit, taxi) system's GOFS 1.0 feeds from its discovery document: `system_information` (default), `service_brands`, `vehicle_types`, `zones` (one row per zone, or a GeoJSON cell with `"params": {"format": "featurecollection"}`), `operating_rules`, `calendars` and `booking_rules`; array-valued fields arrive as JSON strings | Discovery URL (`gofs.json`, required); feed language (default `en`) |
| TODS Operational Data | `tods` | The tables of a TODS v2 archive (`run_events`, `vehicles`, `vehicle_assignments`, `employee_run_dates` and the `*_supplement` files) with the same `list`, `table`, `columns` and `filter` query shape as Static GTFS. With a base GTFS URL configured, `{"table": "trips", "merged": true}` reads a base table with its supplement applied per the TODS evaluation rules (matching rows updated field by field, blank fields ignored, `TODS_delete=1` removed, unmatched rows appended); `trips`, `stops`, `stop_times`, `routes`, `calendar` and `calendar_dates` merge this way | TODS archive URL (`http(s)://`, required); base GTFS archive URL (optional, needed for merged reads); max rows per query (default 100000) |
| WZDx Work Zones | `wzdx` | A WZDx 4.x work zone feed as published by a DOT or a work zone data vendor: `road_events` (default, one row per road event with `core_details` flattened into columns, `road_names`, `types_of_work`, `restrictions` and `related_road_events` as JSON strings, and the geometry columns the map visualization reads), `lanes` (one row per lane per event), `feed_info` and `data_sources`. A device feed, when its URL is configured, serves `devices` (arrow boards, cameras, message signs and the other field devices). `road_events` and `devices` take `"params": {"filter": {"event_type": "detour"}}` against any returned column and `"format": "featurecollection"` for one GeoJSON cell. A 3.x feed is refused with an error naming the version | Work zone feed URL (required); device feed URL (optional); max rows per query (default 5000) |
...
| SoCalTransport | `socaltransport` | Scheduled arrival and departure times by transit node (regional commuter rail, Amtrak, and buses) | Base URL (default provided); the transit node id is a query parameter, not configuration |
