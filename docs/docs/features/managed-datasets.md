...
Every other dataset in the [catalog](/features/data-catalog) got there by being captured: a scheduled query ran, and its results accumulated in the warehouse. A managed dataset works the other way round. Nothing feeds it: an administrator declares its shape, and then people type records into it through the product.
