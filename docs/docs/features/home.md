...
Every section below the search box is **conditional**. A section with nothing to say is not rendered at all, rather than shown as an empty shell, so two people can open Home and see a different number of sections. A brand-new instance shows the greeting and the search box and nothing else.
...
It searches nothing itself. It is the entry point to search, not a second search.
...
A mover backed by a KPI also carries a **freshness badge** underneath it, and it is worth knowing exactly what that badge measures. A counter reading "222 stations, No change" is reassuring, and it can equally mean the number held steady or that no new data arrived at all. The badge is meant to separate those two: it reports the age of the **dataset underneath** the KPI, not when the KPI last recomputed, because a metric sitting on a dead feed goes on recalculating on schedule and reporting itself freshly evaluated.
...
A claim with no page behind it is dropped instead of shown, on the grounds that a claim nobody can check is worse than one that is missing. When every claim is dropped, or the digest is empty, or the request fails, the section simply is not there.

That has a consequence worth knowing: **an absent digest is not a signal.** It looks identical whether AI is switched off, the digest is still warming up, it produced nothing, or every claim was unprovable. If you are checking whether digest generation works, the section's absence will not tell you.
...
**Recent Queries** lists the eight most recently updated queries across the whole org, not just yours, each with its author. It is the one section that shows other people's work by default.
...
Clicking a tag chip anywhere in the product lands here as `?tag=rail`, shown as **Tagged** with the tag beside it and a **Clear tag** link, since a filter you cannot see is one you cannot remove.
...
A failed search says so and offers a retry rather than presenting itself as a search that found nothing. That distinction is not universal across this product, so it is worth knowing that here it holds.
...
The order is deliberate and has two tiers. Anything you have **starred** comes first, then everything else by how recently it changed. Favourites outrank recency absolutely, so a starred dashboard you have not touched in a month still sits above a query edited this morning.
...
The search box narrows the cards on this page. It is a way to pick one out of the shortlist, not a way to find something that is not on it: for that, use [Search](#search), which covers the whole instance and every object type.
...
`/favorites` is everything you have starred, in one place. Stars are per-person: this is your shelf, not the org's.

**Queries** and **Dashboards** are always present. **KPIs** and **Reports** are [enterprise](/editions) sections, and even there they appear only if you have starred one, on the grounds that an empty table under a page already saying you have starred nothing is two answers to the same question.
