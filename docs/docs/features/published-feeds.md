...
Validation is an external service, the containerized MobilityData GTFS-Realtime validator, named by `VEODYN_FEED_VALIDATOR_URL`. Leave it unset and every publish attempt fails closed, recorded as a `failed` attempt.
