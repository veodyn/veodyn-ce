...
The Archive tab matters more here than elsewhere. An archived dashboard appears in no other listing, so this tab is the only route back to one.
...
The header carries the dashboard's editable name, star, tags, and a **Refresh** rate picker (Never, 1 minute, 5, 10, 30 minutes, 1, 12, or 24 hours) that keeps every widget current while the page is open. The rate is a property of your open page, not of the dashboard: close the tab and nothing keeps refreshing.
...
- **Promote to report** ([enterprise](/editions)): copies each widget's query and visualization into a new [report](/features/reports) draft and opens the block editor. It is a **copy, not a link**: the new report carries nothing pointing back here, so the two drift apart from that moment on.
...
`/present/<id>` turns a dashboard into a full-screen deck: one widget per slide, no sidebar, no page chrome, and the browser asked to go fullscreen where it will allow it.
...
Which dashboard is fixed in the instance config as `wall_mode.default_dashboard`. Until that is set, the page says **Wall mode is not configured** and names the setting to change, rather than showing an empty screen someone has to diagnose.
...
It also stops refreshing while the connection is down, and that is deliberate rather than an oversight: a refresh attempted during an outage would come back empty and *replace* the good values on screen with nothing. Freezing the numbers and labelling them is better than a blank wall in a room full of people, so the banner carries the doubt instead.
