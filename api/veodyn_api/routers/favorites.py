...
Its own endpoint group rather than an `isFavorite` field on the objects
themselves: those models are the response of every mutation as well as every
read, so a field on them would have to be resolved on twelve call sites or the
star would flick off after an unrelated edit. The frontend already holds the list
it wants to mark, and one cached read of the id set decides every row.

Queries and dashboards are NOT here: they are Redash objects, and Redash owns
their favorites (`/api/queries/<id>/favorite`).

Every kind comes from the object-type registry, so a kind that is not registered
is refused with a 404 rather than validated against a list this file would have
to keep in step with whatever is installed.
"""
...
def _object_query(org_slug: str, descriptor: registry.ObjectType, object_id: str) -> Select[tuple[str]]:
    ...
    starred, and it is refused with the cause an unregistered kind gets.
    """
...
def add_favorite(object_type: str, object_id: str, identity: IdentityDep, db: DbDep) -> Response:
    ...
    # Nothing written has two causes, a 204 and a 404: the star was already
    # there, or there is nothing to star. Only the second is an error, and it
    # reports the kind's own cause from the descriptor.
...
def remove_favorite(object_type: str, object_id: str, identity: IdentityDep, db: DbDep) -> Response:
    """Remove one star, whether or not the object still exists.
