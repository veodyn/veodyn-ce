"""Response model for the Feed Health endpoint.

The wire shape is the contract in app/src/types/feed.ts. Do not add or
...
class FeedOut(CamelModel):
    """One upstream capture, as the Feed Health board reads it.
    ...
    # "59 seconds ago" and lib/feed-status.ts had no period to age against.
    ...
    # options carry a feed_id label that any Redash user could forge.
    alert_id: int | None
    last_received_at: str
    # Never "down" from here. Down is a verdict about lateness relative to a
    # cadence, and lib/feed-status.ts derives it in the browser against the
