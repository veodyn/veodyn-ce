...
registered. This module fills in the rest from ClickHouse's `system` tables
wherever a provider left it unsaid:

    dataset_sources()    -> id, name, database/table, origin, writability
...
Nothing is invented on top of what a provider states or ClickHouse reports: a
dataset with no rows reports no coverage, and `domain` is null because nothing
here has a notion of one.
"""
...
def _describe(entry: DatasetSource, row_count: int) -> str:
    """Says where the data came from and nothing more.

    A provider's own sentence wins. The ORIGIN, not the presence of a
    description, settles whether a capture sentence may be written at all: a
    managed dataset declared without one holds the empty string, which is falsy.
    """
    if entry.description:
        return entry.description
    if entry.origin != "capture":
        # Nothing truthful left to say about provenance, and the row count is
        # already on the page beside this.
        return ""
...
def dataset_ids(client: ClickHouseClient, default_database: str) -> set[str]:
    ...
    this dataset exist" is answered by asking every registered source. Exposed so
    the tag endpoint can refuse to label a table nobody captured or declared.

    Shadowing is applied so a shadowed source's raw table name, which never
    appears in a catalog response, is not accepted as a tag target either.
    """
...
def apply_shadowing(sources: list[DatasetSource]) -> list[DatasetSource]:
    ...
    under the original name. The registry still returns the renamed table, and
    listing both would put one dataset in the catalog twice under two ids, which
    the frontend resolves by exact match.

    Shadowing a name nobody registered is a no-op: the rename may not have run.
    """
...
def build_catalog(
    ...
    *,
    ...
    # database is in this set, and reading system.columns for it is a wasted
    # round trip on every catalog request.
    ...
    for source in sources:
        ...
        try:
            ...
            # This one table has no `captured_at`, or went away between the
            # registry read and now: unknown coverage for it, and the rest of the
            # response is unaffected. Narrow: any other refusal, a warehouse that
            # is down included, still raises, because an outage rendered as "no
            # coverage yet" is indistinguishable from a fresh install.
            # WarehouseDatabaseMissing is absent from this catch (unlike in
            # capture_sources.py) because a registry row naming a database that
            # does not exist is a deployment fault to surface.
            start, end = None, None
