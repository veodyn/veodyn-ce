"""Captures, derived from the catalog rather than assembled beside it.

A capture here is a scheduled Redash query that writes into the historical
...
Redash connection, not what supplies that connection. So a capture is named
...
how the two surfaces come to disagree about whether a capture is fresh. Deriving
...
# Intervals with an English name the frontend already parses (lib/capture-status.ts
...
# `cadenceToMs`, which returns null, so `deriveCaptureStatus` falls back to the
...
def cadence_label(interval_seconds: int) -> str:
    """A Redash schedule interval as a string lib/capture-status.ts can parse back.
...
def query_facts(
    ...
    Empty is "we could not find out", and build_captures treats it that way: a
    feed with no facts keeps its name, id and last capture, and loses only its
    cadence and source.
...
def build_captures(
    ...
    *,
    ...
    """One capture per captured dataset, in the catalog's own order.
    ...
    for dataset in datasets:
        ...
        captures.append(
            CaptureOut(
                # The dataset id, so freshness.captureId on the catalog side points
