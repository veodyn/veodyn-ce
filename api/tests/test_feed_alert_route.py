"""Arming, moving and disarming a feed's staleness alert, through the route.

Split from test_feeds.py, which is the board's own wire contract and its
degradation behaviour. This is the alert lifecycle: what gets written to Redash,
in what order, and what happens to it when the expectation underneath moves.

The fixtures stay in test_feeds.py and are imported, so the two files cannot
drift into disagreeing about what the deployment looks like. That matters more
than it sounds: the SOURCES fixture describing a deployment with no ClickHouse
data source is what let a probe bound to the wrong runner pass here for as long
as it did.
"""
...
from tests.test_feeds import QUERIES, REDASH, SOURCES, SPANS, redash_lists
...
def arm(catalog_api: TestClient, armed: bool) -> httpx.Response:
    return catalog_api.put(f"/feeds/{RAIL_TABLE}/alert", json={"armed": armed}, cookies={"session": "s"})
...
def test_arming_writes_a_probe_and_an_alert_and_links_them(catalog_api: TestClient) -> None:
    ...
    # Two periods, the same boundary lib/feed-status.ts draws. An alert firing
...
def test_arming_needs_a_declared_expectation_first(catalog_api: TestClient) -> None:
    ...
    response = catalog_api.put(
        "/feeds/historical.q_never_declared_7/alert", json={"armed": True}, cookies={"session": "s"}
    )

    assert response.status_code == 422
    assert response.json()["error"]["id"] == "VEODYN_FEED_NOT_WATCHABLE"
