...
def arm(client: RedashClient) -> tuple[int, int]:
    return capture_alert.arm(
        client,
        capture_id="q_bike_share_stations_1",
        capture_name="Metro stations",
        database="historical",
...
def test_merged_options_drops_the_pre_rename_label() -> None:
    stored = {"feed_id": "historical.q_demo_1", "muted": True}
    merged = capture_alert.merged_options(stored, 3600, "historical.q_demo_1")
    assert "feed_id" not in merged
    assert merged["capture_id"] == "historical.q_demo_1"
