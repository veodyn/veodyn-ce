...
`test_feed_validator.py` is the other half: the report shape, driven straight
through `normalize_report` with no HTTP.
...
from veodyn_api.services.feed_validator import ValidatorUnavailable, validate_feed
