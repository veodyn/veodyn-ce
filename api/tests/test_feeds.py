...
SOURCES = [{"id": 3, "name": "Light Rail"}, {"id": 4, "name": "Bike Share"}]
...
def test_arming_writes_a_probe_and_an_alert_and_links_them(catalog_api: TestClient) -> None:
    ...
    armed = catalog_api.put(f"/feeds/{RAIL_TABLE}/alert", json={"armed": True}, cookies={"session": "s"})
...
def test_moving_the_interval_moves_the_threshold(catalog_api: TestClient) -> None:
    ...
    catalog_api.put(f"/feeds/{RAIL_TABLE}/alert", json={"armed": True}, cookies={"session": "s"})
    catalog_api.put(
        f"/feeds/{RAIL_TABLE}/expectation",
        json={"expectedIntervalSeconds": 3600},
        cookies={"session": "s"},
...
def test_disarming_takes_down_the_alert_then_the_probe(catalog_api: TestClient) -> None:
    ...
    catalog_api.put(f"/feeds/{RAIL_TABLE}/alert", json={"armed": True}, cookies={"session": "s"})
    disarmed = catalog_api.put(f"/feeds/{RAIL_TABLE}/alert", json={"armed": False}, cookies={"session": "s"})
